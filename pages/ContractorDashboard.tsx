        ...
        {project.spaces.map(space => (
          ...
          >
            <div className="h-56 bg-slate-200 relative">
               {space.afterImage && space.beforeImage ? (
                 <BeforeAfterSlider beforeImage={space.beforeImage} afterImage={space.afterImage} />
               ) : (
                 <img src={space.beforeImage || 'https://via.placeholder.com/400'} className="w-full h-full object-cover" alt="Space" />
               )}
...
const StepWizard: React.FC<WizardProps> = ({ mode, onCancel, onSave, initialData, materials }) => {
  ...
  // AI State
  const [generatedAfter, setGeneratedAfter] = useState<string | null>(spaceInfo.afterImage || null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const fullPrompt = `Renovation details: ${spaceInfo.description}. Specific Materials: ${materialsText}. Ensure high photorealism and correct perspective integration of materials.`;
      const base64Data = spaceInfo.beforeImage.includes(',') ? spaceInfo.beforeImage.split(',')[1] : spaceInfo.beforeImage;
      
      const result = await geminiService.generateAfterImage(base64Data, fullPrompt, imageSize);
      setGeneratedAfter(result);
    } catch (e) {
      console.error(e);
  ...
  const handleRefine = async () => {
    if (!generatedAfter || !refinementPrompt) return;
    setLoading(true);
    try {
      const base64Data = generatedAfter.includes(',') ? generatedAfter.split(',')[1] : generatedAfter;
      const result = await geminiService.refineImage(base64Data, refinementPrompt);
      if (result) setGeneratedAfter(result);
      setRefinementPrompt('');
    } catch (e) {
      console.error(e);
  ...
  const handleAutoAnalyze = async () => {
      ...
      try {
        ...
        setSpaceInfo(prev => ({...prev, description: (prev.description ? prev.description + "\n\n" : "") + "AI Analysis: " + analysis}));
      } catch (e) {
        console.error(e);
      }
      setLoading(false);
  ...
  const handleFinalSave = () => {
    ...
    // Navigation is handled by onSave
  };
      ...
      <body>
        ...
        <div class="page">
            ...
            <div class="description">
              ${spaceInfo.description || 'No description provided.'}
            </div>
        ...
        <div class="page">
            ...
            <div class="image-card">
               ...
               <img src="${generatedAfter}" class="main-image" />
            </div>
      ...
      <div className="max-w-3xl mx-auto py-8 px-4">
        ...
        <div className="flex flex-col gap-8">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div className="relative aspect-video w-full overflow-hidden rounded-lg bg-slate-100 mb-4">
                     <img src={spaceInfo.beforeImage!} alt="Before" className="w-full h-full object-cover" />
                </div>
                <button 
                    onClick={handleAutoAnalyze}
                    className="w-full bg-blue-50 text-blue-600 py-3 rounded-lg font-bold hover:bg-blue-100 flex items-center justify-center gap-2 border border-blue-200 transition-colors"
                    disabled={loading}
                >
                    {loading ? (
            ...
            {/* Bottom: Description Input */}
            <div>
               <div className="flex justify-between items-end mb-2">
                   <label className="text-lg font-bold text-slate-800">Scope of Work</label>
                   <button 
               ...
               <div className="relative">
                   <textarea 
                     ...
                     placeholder="Describe the renovation goals... (e.g. Remove the carpet, install oak flooring, paint walls white, add modern lighting)"
        ...
        <div className="flex justify-between items-center pt-8 mt-4">
            ...
            <button 
                ...
                disabled={!spaceInfo.description}
            >
    ...
    return (
      <div className="max-w-6xl mx-auto py-8 px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {/* Selected Materials Sidebar */}
          <div className="w-full lg:w-80 shrink-0">
             <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 sticky top-4">
        ...
        <div className="flex-grow relative flex items-center justify-center p-4">
          {loading ? (
            ...
            <div className="w-full max-w-6xl">
               <BeforeAfterSlider beforeImage={spaceInfo.beforeImage!} afterImage={generatedAfter} />
            </div>
          ) : (
...
export const ContractorDashboard: React.FC<ContractorDashboardProps> = ({ 
  ...
  const navigate = useNavigate();
  // State for projects (initialized from MOCK_PROJECTS)
  const [projects, setProjects] = useState<Project[]>(MOCK_PROJECTS);
  
  // Wizard State - Using local state for mode and selected IDs, but route for visibility
  const [wizardMode, setWizardMode] = useState<'new-project' | 'add-space' | 'edit-space'>('new-project');
  ...
  const handleAddSpace = (projectId: string) => {
    ...
    setWizardMode('add-space');
    navigate('/app/wizard');
  };

  const handleEditSpace = (projectId: string, spaceId: string) => {
    setActiveProjectId(projectId);
    setActiveSpaceId(spaceId);
    setWizardMode('edit-space');
    navigate('/app/wizard');
  };
  ...
  const handleUpdateProjectImage = (id: string, newImage: string) => {
      setProjects(prev => prev.map(p => p.id === id ? { ...p, coverPhoto: newImage } : p));
  };

  const handleWizardSave = (data: any) => {
    let nextPath = '/app';
    
    if (wizardMode === 'new-project') {
       // data is a full project object
       setProjects([data, ...projects]);
       nextPath = `/app/project/${data.id}`;
    } else if (wizardMode === 'add-space') {
       // data is a space object
       setProjects(prev => prev.map(p => {
           if (p.id === activeProjectId) {
               return { ...p, spaces: [...p.spaces, data] };
           }
           return p;
       }));
       if(activeProjectId) nextPath = `/app/project/${activeProjectId}`;
    } else if (wizardMode === 'edit-space') {
        // data is a space object
        setProjects(prev => prev.map(p => {
           if (p.id === activeProjectId) {
               return { ...p, spaces: p.spaces.map(s => s.id === activeSpaceId ? data : s) };
           }
           return p;
       }));
       if(activeProjectId) nextPath = `/app/project/${activeProjectId}`;
  ...
  const handleWizardCancel = () => {
      if (activeProjectId) {
          navigate(`/app/project/${activeProjectId}`);
      } else {
          navigate('/app');
      }
      setActiveProjectId(null);
  ...
  // Helper to get initial data for wizard
  const getWizardInitialData = () => {
      if (wizardMode === 'new-project') return null;
      if (wizardMode === 'add-space') return null; // new space
      if (wizardMode === 'edit-space') {
          const p = projects.find(p => p.id === activeProjectId);
          return p?.spaces.find(s => s.id === activeSpaceId);
      }
      return null;
  };
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
       <Routes>
           ...
           <Route path="wizard" element={
               <div className="animate-fade-in bg-white min-h-[calc(100vh-64px)]">
                   <StepWizard 
                     mode={wizardMode} 
